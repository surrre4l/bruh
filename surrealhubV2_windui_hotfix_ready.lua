...
do
    local ok, result = pcall(function()
        ...
        assert(type(chunk) == "function", "WindUI compile failed: " .. tostring(compileErr))
        return chunk()
    end)
    if not ok or type(result) ~= "table" then
        error("[Surreal] Failed to load WindUI: " .. tostring(result), 0)
    end
    WindUI = result
end
...
-- Use the current official WindUI theme API.  Avoid registering a partial custom
-- theme here because newer WindUI builds validate/use additional theme properties.
local THEME = "Dark"
