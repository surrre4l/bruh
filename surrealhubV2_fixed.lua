--[[
    Surreal Hub V2.0
    Part 1 / 28 — Load, Services, State, Helpers
]]
...
--[[
    Part 2 / 28 — Anti-Kick (Adonis bypass)
]]

-- Executor APIs are optional. Missing APIs must not prevent the UI from loading.
local executorGetGC = rawget(getfenv and getfenv() or _G, "getgc")
local executorFireTouch = rawget(getfenv and getfenv() or _G, "firetouchinterest")
local executorFireClick = rawget(getfenv and getfenv() or _G, "fireclickdetector")
...
--[[
    Part 6 / 28 — Main · Extras + Comebacks + Fly
]]
...
--[[
    Part 7 / 28 — Main · Nameplates + Hacker Detector
]]
...
--[[
    Part 9 / 28 — Camp · Challenges (2/2) + Spleef Godmode
]]
...
--[[
    Part 12 / 28 — Morphs Tab (with Symbol Picker)
]]
...
MorphsTab:Dropdown({
    ...
    Callback = safe(function(idx)
        ...
        else pick = SYMBOL_ORDER[tonumber(idx) or 1] end
...
--[[
    Part 14 / 28 — Visuals · Shaders (Snug + Slaughter + Ella)
]]
...
--[[
    Part 15 / 28 — Visuals · Fullbright + Finish ESP + Custom
]]
...
--[[
    Part 16 / 28 — Visuals · ESP + Usernames
]]
...
--[[
    Part 17 / 28 — Themes Tab (Map Themes)
]]
...
--[[
    Part 18 / 28 — Troll Tab (Follower + Scripts + Sounds)
]]
...
--[[
    Part 19 / 28 — Utilities · World + Collision + Gear Keeper
]]
...
--[[
    Part 20 / 28 — Utilities · Gear Spawner + Info + Teleports + Loaders + Interface + Admin Panel
]]
...
UtilitiesTab:Dropdown({
    ...
    Callback = safe(function(idx)
        local list = listGear()
        local pick = list[tonumber(idx) or 1]
...
statsDropdown = StatsTab:Dropdown({
    ...
    Callback = safe(function(idx)
        ...
        local pick = opts[tonumber(idx) or 1]
...
--[[
    Part 23 / 28 — Character Tab (Skin Changer + Marshmallow + Elim)
]]
...
CharacterTab:Dropdown({
    ...
    Values = skinLabels,
    Callback = safe(function(idx)
        local label = skinLabels[tonumber(idx) or 1]
...
CharacterTab:Dropdown({
    ...
    Values = marshList,
    Callback = safe(function(idx)
        local name = marshList[tonumber(idx) or 1]
...
CharacterTab:Dropdown({
    ...
    Callback = safe(function(idx)
        local pick = ({ "None", "Sleigh Elimination" })[tonumber(idx) or 1]
...
--[[
    Part 25 / 28 — Autoplay + Recorder (TAS)
]]
...
if #runs > 0 then
    ...
    AutoplayTab:Paragraph({
        Title   = "No runs",
        Content = "Could not fetch manifest.",
    })
end
...
SettingsTab:Dropdown({
    ...
    Callback = safe(function(idx, label)
        local pick = label
            or ({ "Surreal", "Dark", "Light", "Aqua", "Bloom", "Rose" })[tonumber(idx) or 1]
...
pcall(function() if Window and Window.Init then Window:Init() end end)
